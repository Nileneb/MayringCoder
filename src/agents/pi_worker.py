...
def _claim_one(
    *,
    ...
    (memory hooks) starved into 9s timeouts. We gate each claim on a free
    worker slot: concurrency is capped at the executor size and the claim rate
    can never outrun job completion. The slot is released by the wrapped
    completion callback (or eagerly on busy/empty/submit-failure) — no leak.
    """
