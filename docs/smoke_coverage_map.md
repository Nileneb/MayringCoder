...
| # | Issue | Coverage |
|---|---|---|
| 184a | Rationale-Edges (WHY-marker → wiki_edges → /memory/search co-injection) | Pytest `tests/test_rationale_parser.py` (6 tests: single-line, multi-line, qualified-class, async, skip-non-trivial, repo-walk) + `tests/test_memory_retrieval.py::test_search_attaches_rationale_edges` + `test_compress_for_prompt_renders_rationale_block` (Red-Green TDD verifiziert) — Spec `docs/superpowers/specs/2026-05-09-rationale-edges-design.md`, Plan `docs/superpowers/plans/2026-05-09-rationale-edges.md`, Commits afdd86a/32a15dc/8f7b564/46759d7/9adf3a9/e2fe6f6/c9db1bf/f5435d5 |
