...
| 195-Iter4 | Public-Share-Endpoint | `POST /sources/{id}/share` (`src/api/routes/memory.py`) — no body → visibility='public'; `{org_id}` → visibility='org' (member-checked); same L8 owner-check as PATCH /visibility. `ShareSourceRequest` model. Pytest `tests/test_shared_memory.py` (3 share tests) + smoke `check_share_endpoint`. Combined visibility coverage stays in `check_visibility_isolation` (private+public ingest+patch+search). |
