...
| 340 | smoke FAIL `reranker_cat_match_fires` — leere codebook_categories-Chroma nach Stack-Recreate | **API** — `reranker_cat_match_fires` in `tools/smoke_test_production.py`. Root-Cause: die `codebook_categories`-Chroma-Collection verliert beim Stack-Recreate ihre Vektoren → query→category leer → cat_match inert. Repariert via `POST /stats/admin/reembed-categories` (idempotenter Upsert); cat-match-debug bestätigt `chroma_codebook_count=1629`, query→category_ids derived. Reembed ist jetzt gefahrlos (Hotpath durch #343 gehärtet). |
