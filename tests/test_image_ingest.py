"""Tests for image ingest (src/memory_ingest.py)."""
...
class TestRunImageIngest:
    def test_clones_repo_and_ingests_images(self, tmp_path):
        ...
        with (
            patch("mayring_core.memory.image_ingest.subprocess.run", return_value=MagicMock(returncode=0)),
            patch("mayring_core.memory.image_ingest.tempfile.mkdtemp", return_value=str(tmp_path)),
            patch("mayring_core.memory.image_ingest.shutil.rmtree") as mock_rmtree,
            patch("mayring_core.memory.ingest.ingest_image", mock_ingest_image),
            patch("mayring_core.memory.image_ingest.init_memory_db", return_value=mock_conn),
    ...
    def test_cleanup_runs_even_on_error(self, tmp_path):
        with (
            patch("mayring_core.memory.image_ingest.subprocess.run", side_effect=RuntimeError("clone failed")),
            patch("mayring_core.memory.image_ingest.tempfile.mkdtemp", return_value=str(tmp_path)),
            patch("mayring_core.memory.image_ingest.shutil.rmtree") as mock_rmtree,
        ):
    ...
    def test_returns_correct_counts(self, tmp_path):
        ...
        with (
            patch("mayring_core.memory.image_ingest.subprocess.run", return_value=MagicMock(returncode=0)),
            patch("mayring_core.memory.image_ingest.tempfile.mkdtemp", return_value=str(tmp_path)),
            patch("mayring_core.memory.image_ingest.shutil.rmtree"),
            patch("mayring_core.memory.ingest.ingest_image", mock_ingest_image),
            patch("mayring_core.memory.image_ingest.init_memory_db", return_value=MagicMock()),
